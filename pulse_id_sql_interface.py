                ...
                if st.button(f"Generate Emails For Above Extracted Merchants", key=f"generate_emails_{idx}"):
                    with st.spinner("Generating emails..."):
                        try:
                            ...
                            llm_email = LLM(model="llama3-70b-8192", api_key=st.session_state.api_key)
                            email_agent = Agent(
                            ...
                            task = Task(
                                ...
                                agent=email_agent,
                                expected_output="Marketing emails for each selected merchant, tailored to their business details. Please use a lines to seperate each emails if there is more than one"
                            )
